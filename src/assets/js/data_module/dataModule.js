var dataModule = (function (d3) {
  ...
  function _prepareGroupsObject(dataToUse, mainHub) {
    ...
    dataToUse.forEach(function (node) {
      if (
        node.ptype.toLowerCase() !== varConfig.SPOKE &&
        node.ptype.toLowerCase() !== varConfig.SUBSPOKE &&
        !newGroup.hasOwnProperty(`group_${node.pid}`)
      ) {
  ...
  function _findHub(spokeToUse, dataToUse, mainHub) {
    ...
    if (previousNode == null) {
      ...
      previousNode.ptype.toLowerCase() == varConfig.HUB ||
      previousNode.ptype.toLowerCase() == varConfig.SUBHUB
    ) {
      return previousNode;
  ...
  function _assignDynamicCoordinates(dataToUse, mainRadius, graphWidth, graphHeight, mainHub) {
    ...
    Object.keys(groupedGraph).forEach(function (group) {
      if (groupedGraph[group].type == varConfig.HUB) {
        ...
        groupedGraph[group].radius =
          Math.floor(Math.random() * (mainRadius + varConfig.DEAFULT_SUBGRAPH_RADIUS.OFFSET_MIN - mainRadius + 1)) +
          mainRadius;
      }
    });
  ...
  function createAxisDynamic(mainRadius, originaldata, gcWidth, gcHeight) {
    ...
    if (Array.isArray(originaldata) && originaldata.length) {
      ...
      if (firstHub) {
        ...
        let newCoordinates = _assignDynamicCoordinates(
          originaldata,
          mainRadius,
          gcWidth,
          gcHeight,
  ...
  var getData = function (video_id, graphContainerHeight, graphContainerWidth, interactionsRawData, cb) {
    console.log('video id recieved is ', video_id);
    if (d3) {
      // './../../data/multiple_videos.json'
      //d3.json('/assets/json/vis_db.json', (err, data) => {
      console.log(interactionsRawData);
      var data = JSON.parse(interactionsRawData);
      if (data) {
        ...
        if (fetchedData !== -1) {
          ...
          console.log('whole data: ', JSON.stringify(originaldata));
